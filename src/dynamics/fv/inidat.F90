...
!
    ...
    character(len=16) :: subname='READ_INIDAT'   ! subroutine name
...
!-----------------------------------------------------------------------
!
    use constituents, only: cnst_name, qmin, cnst_get_ind
...
!
	  ...
	  call get_block_gcol_d(iam+1,blksiz,gcid)
          ...
          if (microp_driver_implements_cnst(cnst_name(m_cnst))) then
            ...
            allocate( qqtmp(blksiz,plev),stat=ierror )
            allocate( qltmp(blksiz,plev),stat=ierror )
            allocate( qitmp(blksiz,plev),stat=ierror )
            ...
            do k=1,km
               ...
               do j=jfirstxy,jlastxy
                  do i=ifirstxy,ilastxy
                     ib=ib+1
                     qqtmp(ib,k) = tracer(i,j,k,1)
                     qltmp(ib,k) = tracer(i,j,k,ixcldliq)
                     qitmp(ib,k) = tracer(i,j,k,ixcldice)
                  end do
               end do
            end do

            call wtrc_init_cnst(cnst_name(m_cnst),q3tmp, gcid, qqtmp, qltmp, qitmp)
